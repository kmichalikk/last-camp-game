...
class VisualHalf:
	...
	func update_points(points: Array[Vector3], origin: Node3D):
		curve.clear_points()
		for p in points:
			curve.add_point(origin.to_local(p))
...
func update_rope(points: Array[Vector3], broken: bool):
	if points.size() < 2: 
		return
